...
public class GourmetCoffee  {
	...
	public void displayNumberOfOrders(Product product) {
		...
		int cnt = 0; // count
		for(Order i: sales)
			if(i.getItem(product) != null)
				cnt++;
		System.out.println("Number of orders that contains the product " + product.getCode() + ": " + cnt);
		System.out.println();
	}
	...
	public void displayTotalQuantityOfProducts() {
		...
		for(Product i: catalog) {
			...
			for(Order j: sales)
				if(j.getItem(i) != null)
					cnt += j.getItem(i).getQuantity();
			System.out.println(i.getCode() + " " + cnt);
		}
	}
