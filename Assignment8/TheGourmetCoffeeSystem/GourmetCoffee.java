...
public class GourmetCoffee  {
	 ...
	 *
	 * [error here] initialCatalog a product catalog
	 */
	...
	private void displaySales() {
		System.out.print(salesFormatter.formatSales(sales));
	}
	...
	private void run() throws IOException  {
		...
		while (choice != 0)  {
			SalesFormatter formatter = null;
			if (choice == 1)  {
				formatter = PlainTextSalesFormatter.getSingletonInstance();
			} else if (choice == 2)  {
				formatter = HTMLSalesFormatter.getSingletonInstance();
			} else if (choice == 3)  {
				formatter = XMLSalesFormatter.getSingletonInstance();
			}
			if(formatter != null) {
				setSalesFormatter(formatter);  //mdf
